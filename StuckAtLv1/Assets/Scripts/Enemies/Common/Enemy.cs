...
*/
{
    ...
    private void OnEnable() {
        if (FindObjectOfType<MapManager>(includeInactive: true).GetWorld() == 3 && FindAnyObjectByType<FinalBossManager>().GetPhase() == 1) {    //change later to 5 for full game
            finalBoss = true;
        } else {
            finalBoss = false;
        }
        ...
        if (hp < 1) {
            if (!tutorial) {
                ...
                if (c.GetObjective() == "miniboss" && (gameObject.CompareTag("Knight") || gameObject.CompareTag("DeerNymph"))) {
                    c.EnemyKilled();
                } else if (c.GetObjective() == "boss" && (gameObject.CompareTag("Lich") || gameObject.CompareTag("VenusFlyTrap") || gameObject.CompareTag("FinalBossPhase3")) ) {
                    c.EnemyKilled();
